...
<main 
    ...
    on:keydown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleDescription()} 
    tabindex="0" 
    class="p-6 flex flex-col gap-4 rounded-lg border border-solid border-purple-700 text-center cursor-pointer group hover:border-purple-400 duration-200"
>
    {#if product.isSoldOut}
        <div class="absolute top-0 left-0 bg-gray-500 text-white px-2 py-1 text-xs">SOLD OUT</div>
    {/if}
    <div class="bg-black grid place-items-center px-4 text-5xl md:text-6xl mx-auto duration-200 transform group-hover:scale-105">
        <img src={product.image} alt={product.name} class="w-full h-48 object-cover mb-4 transition-transform duration-300 rounded-lg" />
    </div>
    <h3 class="font-medium text-2xl font-caveat">
        {product.name}
    </h3>
    ...
    {#if showDescription}
        <div class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
            <div class="bg-slate-950 p-8 rounded-lg shadow-2xl text-white max-w-2xl w-full mx-4 flex flex-col md:flex-row">
                <img src={product.image} alt={product.name} class="w-full md:w-1/2 h-auto object-cover mb-4 md:mb-0 rounded-lg" />
                <div class="md:ml-6">
