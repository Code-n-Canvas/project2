...
describe('Form Validation', () => {
  ...
  it('should return an error for invalid email', () => {
    const errors = validateForm('John Doe', 'invalid-email', '1234567890', '123 Main St', 'Inquiry');
    expect(errors.email).toBe("Email is invalid.");
  });

  it('should return an error for invalid phone number', () => {
    const errors = validateForm('John Doe', 'john@example.com', '12345', '123 Main St', 'Inquiry');
    expect(errors.phoneNumber).toBe("Phone number must be exactly 10 digits long.");
  });

  it('should return an error for missing reason', () => {
    const errors = validateForm('John Doe', 'john@example.com', '1234567890', '123 Main St', '');
    expect(errors.reason).toBe("Reason for contact is required.");
  });

  it('should return no errors for valid inputs', () => {
    const errors = validateForm('John Doe', 'john@example.com', '1234567890', '123 Main St', 'Inquiry');
    expect(errors).toEqual({});
  });
});